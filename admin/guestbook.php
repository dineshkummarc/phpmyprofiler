...
if ( !empty($_GET['id']) ) {
	switch ($_GET['action']) {
		case 'delete':
			$sql = 'DELETE FROM pmp_guestbook WHERE id = ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
			$smarty->assign('Success', t('Guestbook entry deleted.'));
			break;

		case 'activate':
			$sql = 'UPDATE pmp_guestbook SET status = 1 WHERE id = ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
			$smarty->assign('Success', t('Guestbook entry activated.'));
			break;

		case 'comment':
			$sql = 'UPDATE pmp_guestbook SET comment = \'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['comment'])
			. '\' WHERE id = ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
