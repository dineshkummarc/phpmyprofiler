...
function updateRates() {
	...
	if ( $xmlrates != '' ) {
		...
		foreach ( $rates as $curr=>$rate ) {
			dbexec('INSERT INTO pmp_rates (id, rate, date) VALUES (\'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $curr) . '\', ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $rate) . ', \'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $date[1]) . '\')');
		}
