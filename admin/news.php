...
if ( isset($_GET['action']) ) {
	switch ($_GET['action']) {
		case 'delete':
			if ( !empty($_GET['id']) ) {
				$sql = 'DELETE FROM pmp_news WHERE id = ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
				$res = dbexec($sql);
		...
		case 'addsave':
			if ( (empty($_POST['title'])) || (empty($_POST['text'])) ) {
				...
				$sql = 'INSERT INTO pmp_news (date, title, text) VALUES (now(), \'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['title']) . '\', \''
				. mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['text']) . '\')';
				$res = dbexec($sql);
		...
		case 'edit':
			if ( !empty($_GET['id']) ) {
				$sql = 'SELECT * FROM pmp_news WHERE id = ' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
				$res = dbexec($sql);
		...
		case 'editsave':
			if ( !empty($_GET['id']) ) {
				$sql = 'UPDATE pmp_news SET title = \'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['title']) . '\', text = \''
				. mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['text']) . '\', date = now() WHERE id = '
				. mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
				$res = dbexec($sql);
