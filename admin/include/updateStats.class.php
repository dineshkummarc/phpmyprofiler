...
class updateStats {
	...
	public function update() {
		...
		if ( $pmp_exclude_tag != '' ) {
			$query .= " WHERE pmp_film.id NOT IN (SELECT id FROM pmp_tags where name = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $pmp_exclude_tag) . "')";
		}
		...
		if ( isset( $pmp_collections) ) {
			foreach ( $pmp_collections as $collection ) {
				...
				if ( $pmp_exclude_tag != '' ) {
					$query .= " AND pmp_film.id NOT IN (SELECT id FROM pmp_tags where name = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $pmp_exclude_tag) . "')";
				}
				$result = dbexec($query);
		...
		if ( count($usedcurrencies) > 0 ) {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				$sql .= "('highest_price', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->title) . "', '" . $row->id . "', '". $row->purchprice . "'),";
			}
		}
		...
		if ( count($usedcurrencies) > 0 ) {
			...
			for ($i = 0; $i < $count; $i++) {
				...
				$result = dbexec($query);
				$row = mysqli_fetch_object($result);
				$sql .= "('lowest_price', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->title) . "', '" . $row->id . "', '". $row->purchprice . "'),";
			}
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('longest', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->title) . "', '".$row->id  ."', '" . $row->runningtime . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('shortest', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->title) . "', '".$row->id  ."', '" . $row->runningtime . "'),";
		}
		...
		while ( $row = mysqli_fetch_object($result) ) {
			...
			$sql .= "('purchplace_top10', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->purchplace) . "', '" . $sum . "', '" . $row->quantity . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('genres_top10', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->genre) . "', '', '" . $row->quantity . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('studios_top10', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->studio) . "', '', '" . $row->quantity . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('company_top10', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->company) . "', '', '" . $row->quantity . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('origins', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->country) . "', '', '" . $row->quantity . "'),";
		}
		...
		$result = dbexec($query);
		while ( $row = mysqli_fetch_object($result) ) {
			$sql .= "('localities', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row->locality) . "', '', '" . $row->quantity . "'),";
		}
