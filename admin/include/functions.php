...
function saveLastIP() {
	...
	dbexec($sql);
	// Insert new IP
	$sql = "INSERT INTO pmp_statistics VALUES ('last_login_new', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_SERVER['REMOTE_ADDR']) . "', '" . date("H:i") . "', '". date("Y-m-d") . "');";
	dbexec($sql);
	dbclose();
}
...
function maskData($value) {
	if ( get_magic_quotes_gpc() == true ) {
		$maskedData = mysqli_real_escape_string($GLOBALS["___mysqli_ston"], stripslashes($value));
		return $maskedData;
	}
	else {
		$maskedData = mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $value);
		return $maskedData;
	}
}
...
function genScreenshotTag($indb) {
	...
	foreach ($indb as $id) {
		$sql = "INSERT INTO pmp_tags values ('" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $id['id']) . "', 'Screenshots', 'Screenshots')";
		dbexec($sql, true);
	}
...
function getScreenshotsAdm($path) {
	...
	dbconnect();
	foreach ($ids as $id) {
		$sql = "SELECT id, title, sorttitle FROM pmp_film WHERE id='".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $id)."'";
		$res = dbexec($sql);
		if ( mysqli_num_rows($res) != 0 ) {
			$row = mysqli_fetch_object($res);
