...
function writeAward() {
	...
	$del .= "(award='" . trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], stripslashes($awards['AWARD']))) . "' AND awardyear='" . trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $awards['YEAR'])) . "') OR ";
	...
	for ($i = 1; $i <= count($awards['CATEGORY']); $i++) {
		...
		for ($p = 1; $p <= count($awards['CATEGORY'][$i]['MOVIE']); $p++) {
			...
			$insert .= "('".trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], stripslashes($awards['CATEGORY'][$i]['MOVIE'][$p]['TITLE']))) . "','"
			. "','"
			. trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], stripslashes($awards['AWARD']))) . "','"
			. trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $awards['YEAR'])) . "','"
			. trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $awards['CATEGORY'][$i]['NAME'])) . "','"
			. $win . "','"
			. trim(mysqli_real_escape_string($GLOBALS["___mysqli_ston"], getIsset($awards['CATEGORY'][$i]['MOVIE'][$p]['NOMINEE']))) . "'),";
		}
	}
...
if ( (isset($_GET['action'])) && ($_GET['action'] == "deleteaward") ) {
	if ( isset($_GET['award']) ) {
		...
		// Delete existing awards
		$del = 'DELETE FROM pmp_awards WHERE award = \'' . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['award']) . '\';';
		dbexec($del, true);
	}
}
...
if ( (isset($_GET['action'])) && ($_GET['action'] == "buildtags") ) {
	if ( isset($_GET['award']) ) {
		...
		$sql = "SELECT DISTINCT id FROM pmp_film LEFT JOIN pmp_awards ON LOWER(pmp_film.originaltitle) = LOWER(pmp_awards.title) WHERE award = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['award']) . "' AND winner ='1' AND originaltitle != '' AND awardyear BETWEEN pmp_film.prodyear-1 AND pmp_film.prodyear+1;";
		$res = dbexec($sql);
		$count = mysqli_num_rows($res);
		while ($row = mysqli_fetch_object($res)) {
			if ( empty($insert) ) $insert = "INSERT INTO pmp_tags VALUES ";
			$insert .= "('" . $row->id . "', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['award']) . "', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['award']) . "'),";
		...
		// Get winner by title
		$sql = "SELECT DISTINCT id FROM pmp_film LEFT JOIN pmp_awards ON LOWER(pmp_film.title) = LOWER(pmp_awards.title) WHERE award = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['award']) . "' AND winner ='1' AND originaltitle = '' AND awardyear BETWEEN pmp_film.prodyear-1 AND pmp_film.prodyear+1;";
		$res = dbexec($sql);
