...
if ( (isset($_GET['action'])) && ($_GET['action'] == 'delete') ) {
	if ( !empty($_GET['id']) ) {
		$sql = "SELECT filename FROM pmp_pictures WHERE id = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']) . "'";
		$res = dbexec($sql);
		$row = mysqli_fetch_object($res);
		@unlink('../pictures/' . $row->filename);

		$sql = "DELETE FROM pmp_pictures WHERE id = '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']) . "'";
		$res = dbexec($sql);
				...
				if ( $ext !== false ) {
					...
					if ( !file_exists('../pictures/' . $name) ) {
						if ( move_uploaded_file($_FILES['picture']['tmp_name'], '../pictures/' . $name) ) {
							...
							$sql = "INSERT INTO pmp_pictures (filename, title) VALUES ('" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $name)
							. "', '" . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['title']) . "')";
							$res = dbexec($sql);
