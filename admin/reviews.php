...
if ( !empty($_GET['id']) ) {
	switch ($_GET['action']) {
		case "delete":
			$sql = "SELECT * FROM pmp_reviews WHERE id = " . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
			$row = mysqli_fetch_object($res);
			$sql = "DELETE FROM pmp_reviews WHERE id = " . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
		...
		case "activate":
			$sql = "UPDATE pmp_reviews SET status = 1 WHERE id = " . mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['id']);
			$res = dbexec($sql);
